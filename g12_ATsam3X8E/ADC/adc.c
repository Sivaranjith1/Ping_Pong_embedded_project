...
void adc_init(void){
    ...
    //ADC->ADC_MR = ADC_MR_TRGSEL_ADC_TRIG0 | ADC_MR_TRGEN_DIS | ADC_MR_TRACKTIM(1) | ADC_MR_STARTUP_SUT8; //Prescaler = 0, Timer enabled for CH0, Tracking time 2 periods, 8 periods SUT
    ...
    NVIC_EnableIRQ(ID_TC0); // Enable interrupt for timer channel 0
    NVIC_EnableIRQ(ID_ADC); //Enable NVIC interupt
    */
