...
static uint8_t oled_read_previous_data(void){
	return xmem_read(OLED_SRAM_ADDRESS_START + 128*current_page + current_column);
}

void oled_redraw(char data){
	oled_write_data(data);
	/*char old_state = oled_read_previous_data();
	if(old_state != data){
